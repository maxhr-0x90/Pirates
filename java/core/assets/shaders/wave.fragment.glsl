...
void main() {
    vec2 texCV = mod(vec2(v_texCoord0.x, v_texCoord0.y + u_time * 0.01) * 5.0, 1.0);
    vec2 texCH = mod(vec2(v_texCoord0.x + u_time * 0.02, v_texCoord0.y) * 2.5, 1.0);
    vec3 normV = texture2D(u_ambientTexture, texCV).xyz * 2.0 - 1.0;
    vec3 normH = texture2D(u_normalTexture, texCH).xyz * 2.0 - 1.0;
